...
### [DONE] Backlog Item 0: v2.8.0 Zero-Context SSoT Governance Scaffold
- **작업 내용**:
  - `SSOT_MAP.md`를 통한 도메인별 기준 문서 확립
  - `STAKEHOLDER_INTENT.md`를 통한 인스트럭터 및 AI 에이전트 목표 정의
  - `PROJECT_STATE.json` 시스템 상태 스냅숏 생성
- **비고**: Scaffold merged; 실행 계층은 v2.8.1 Hotfix에서 완성.

### [ACTIVE] Backlog Item 1: v2.8.1 Governance Hardening Hotfix (외부 리뷰 P0/P1 반영)
- **작업 내용**:
  - Review 상태 머신 강제 (`review_state.py`, ALLOWED_TRANSITIONS, fail-closed + non-zero exit)
  - Append-only `teacher_review_event` 감사 로그 + `review_version` 낙관적 잠금
  - Claim-level Provenance 영속화 (`claim_provenance` 테이블/모듈, 빈 axis 합성 제거)
  - DDL SSoT 동기화 (`Taxonomy_Spec.md` v2.8.1, CHECK 제약), MANIFEST↔PROJECT_STATE 모순 제거
  - SSoT Consistency Validator + Governance CI (`.github/workflows/governance-ci.yml`, py3.10/3.12)
- **Acceptance Gate**: 원격 CI green PR 병합 시 `teacher_governance_loop: ACTIVE` 선언 가능

### [PLANNED] Backlog Item 2: v2.9.0 Technical Proof & Independent Solvers
- **작업 내용**:
...
- [x] **Phase 3-B**: **v2.8.0** Zero-Context SSoT Governance Scaffold (완료)
- [~] **Phase 3-B'**: **v2.8.1** Governance Hardening Hotfix (구현 완료, 원격 CI Proof 대기)
- [ ] **Phase 3-C**: **v2.9.0** Technical Proof & Independent Solvers 연동 (예정)
